...
def on_new_sample(sink: int) -> Gst.FlowReturn:
...
def play_pipewire_stream(node_id):
    ...
    pipeline = Gst.parse_launch(
        'pipewiresrc fd=%d path=%u ! videoconvert ! pngenc ! appsink name=sink'%(fd, node_id))
...
# Объявление глобальной переменной для хранения изображения
current_pipeline = None
image_data = None  # Переменная для хранения изображения в оперативной памяти
